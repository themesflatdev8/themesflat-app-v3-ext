import api from "@/api";
import { FEATURE_TYPE_PAYMENT_BADGES_ON_CART } from "@/constants/features";
import { getVariablesStylePaymentBadges } from "@/shared/variables-style";
import { getImgFromServer } from "@/utils/cdn";
import { _checkoutButton } from "@/constants/selectors";

export default class PaymentBadgesOnCart {
  constructor(payload) {
  ...
  load() {
    ...
    this.checkoutSelector?.forEach((keyData) => {
      ...
      const paymentCart = document.querySelectorAll(".Msell-Payment-Badge--Cart");
      let check = 0;
  ...
  renderHtml({ el, data }) {
    ...
    el.insertAdjacentHTML(
      ...
      `
      <div class="Msell-Payment-Badge Msell-Payment-Badge--Cart" data-template="${data.template}" style="${styles}">
        <h2>${data.heading || ""}</h2>
