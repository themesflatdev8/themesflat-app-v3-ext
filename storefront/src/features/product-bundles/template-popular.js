import viewWidget from "@/views/product-bundles/template-popular/index.handlebars";
import viewProductItem from "@/views/product-bundles/template-popular/product-item.handlebars";
...
export default class BundlePopular {
  constructor(payload) {
  ...
  initialize() {
    ...
    let list = self.widget.default
      ...
      : [...listRecommend];
    self.widget.el.classList.add("Msell-BPopular");
    self.handleMedia();
    ...
    self.widget.el.setAttribute("id", `Msell-BPopular-${self.widget.widgetID}`);
    const elList = self.widget.el.querySelector(".Msell-BPopular__List");
    self.renderProducts({
      list,
      elList,
    });
  ...
  handleMedia() {
    ...
    self.widget.el.classList.remove(
      "Msell-BPopular--Desktop",
      "Msell-BPopular--Tablet",
      "Msell-BPopular--Mobile",
      "Msell-BPopular--Mobile-LG",
      "Msell-BPopular--Mobile-SM"
    );
    let offsetWidthParent = self.widget.el.parentElement?.offsetWidth || 0;
    if (offsetWidthParent > 1152) {
      self.widget.el.classList.add("Msell-BPopular--Desktop");
    } else if (offsetWidthParent > 950) {
      self.widget.el.classList.add("Msell-BPopular--Desktop");
    } else if (offsetWidthParent > 750) {
      self.widget.el.classList.add("Msell-BPopular--Tablet");
